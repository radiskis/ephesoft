...
public class ApplicationContextUtil {
	...
	public static <T> Map<String, T> initializeBeans(ConfigurableApplicationContext context, Resource res) {
		BeanDefinitionRegistry r = (BeanDefinitionRegistry) context.getBeanFactory();

		XmlBeanFactory f = new XmlBeanFactory(res);
		String[] names = f.getBeanDefinitionNames();
		for (String n : names) {
			r.registerBeanDefinition(n, f.getBeanDefinition(n));
		}
	...
	public static <T> Map<String, T> getBeansFromContext(BeanFactory bf, String[] names) {
		Map<String, T> beans = new HashMap<String, T>(names.length);
		for (int i = 0; i < names.length; i++) {
			String n = names[i];
			beans.put(n, (T) bf.getBean(n));
		}
		return beans;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> T getBeanFromContext(BeanFactory bf, String name, Class<T> type) {
	...
	public static <T> Collection<T> getBeansOfType(ApplicationContext ctx, Class<T> type) {
		Map<String, T> m = ctx.getBeansOfType(type);
		return m.values();
	}
