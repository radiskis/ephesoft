...
public class CustomValueSortedMap extends TreeSet<CustomMapClass> {
	...
	@Override
	public boolean add(CustomMapClass e) {
			...
			if (super.size() == maxValue) {
				if (super.add(e)) {
					...
					while (itr.hasNext()) {
						...
						if (customMapClass.equals(e)) {
							if (e.getValue().compareTo(customMapClass.getValue()) > 0) {
...
class DoubleComparator implements Comparator<CustomMapClass> {
	...
	@Override
	public int compare(CustomMapClass o1, CustomMapClass o2) {
		int returnValue = o1.getValue().compareTo(o2.getValue());
		if (returnValue == 0) {
			return -1;
		}
