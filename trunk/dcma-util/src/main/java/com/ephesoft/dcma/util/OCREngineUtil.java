...
public class OCREngineUtil {
	...
	protected static Log log = LogFactory.getLog(OCREngineUtil.class);
	...
	public static void formatHOCRForTesseract(final String outputFilePath, final String actualFolderLocation) throws Exception {
		...
		org.w3c.dom.Document doc2 = XMLUtil.createDocumentFrom(inputStream);
		...
		for (int pageNodeIndex = 0; pageNodeIndex < pageList.getLength(); pageNodeIndex++) {
			...
			String pageID = ((Node)pageNode.getAttributes().getNamedItem("id")).getTextContent();
