...
public class FuzzySearchListView {
	...
	public FuzzySearchListView(List<String> arg0) {

		List<HeaderColumn> headerColumns = new ArrayList<HeaderColumn>();

		int widthOfHeaderCol = 94 / arg0.size();
		int index = 0;
		for (String str : arg0) {
			HeaderColumn headerColumn = new HeaderColumn(index, str, widthOfHeaderCol);
			index++;
			headerColumns.add(headerColumn);
		}

		HeaderColumn action = new HeaderColumn(index, "", 1);

		headerColumns.add(action);

		listView.addHeaderColumns(headerColumns);
	}
}
