...
public class PreImageLoadingPresenter implements Presenter {
	...
	public PreImageLoadingPresenter(ReviewValidateDocServiceAsync rpcService, HandlerManager eventBus,
			ReviewValidatePresenter reviewValidatePresenter) {
		this.rpcService = rpcService;
		this.eventBus = eventBus;
		this.reviewValidatePresenter = reviewValidatePresenter;
		this.injectEvent(eventBus);
	}
	...
	public void loadInvalidDocuments(String batchId, int numberOfInvalidDocs) {
		Batch batch = reviewValidatePresenter.batchDTO.getBatch();
		List<Document> documentList = new ArrayList<Document>();
		Documents documents = batch.getDocuments();
		if (documents != null) {
			List<Document> docList = documents.getDocument();
			switch (reviewValidatePresenter.batchDTO.getBatchInstanceStatus()) {
				case READY_FOR_REVIEW:
					// Loading un reviewed document.
					for (Document document : docList) {
						if (document != null && !document.isReviewed()) {
							if (documentList.size() < numberOfInvalidDocs) {
				...
				case READY_FOR_VALIDATION:
					...
					// Loading invalidated document
					for (Document document : docList) {
						if (document != null && (!document.isReviewed() || !document.isValid())) {
							if (documentList.size() < numberOfInvalidDocs) {
								documentList.add(document);
							} else {
								break;
