...
public class KV_PP_PropertiesPresenter extends AbstractBatchClassPresenter<KV_PP_PropertiesView> {
	...
	public KV_PP_PropertiesPresenter(BatchClassManagementController controller, KV_PP_PropertiesView view) {
		...
		this.kvPPDetailPresenter = new KV_PP_DetailPresenter(controller, view.getKvPPDetailView());
		this.kvPPEditPresenter = new KV_PP_EditPresenter(controller, view.getKvPPEditView());
	...
	public void bind() {
		if (controller.getSelectedPlugin() != null) {
			...
			if (values == null || values.isEmpty()) {
				...
				view.getNoResultLabel().setVisible(true);
			} else {
				view.getEditButton().setVisible(true);
				view.getNoResultLabel().setVisible(false);
			}
			view.getEditButton().addClickHandler(new ClickHandler() {

				@Override
				public void onClick(ClickEvent arg0) {
					view.getKvppPluginViewVerticalPanel().setVisible(Boolean.FALSE);
