...
public class EditPluginView extends View<EditPluginPresenter> {
	...
	interface Binder extends UiBinder<FlexTable, EditPluginView> {
	}
	...
	public ValidatableWidget<TextBox> addTextBox(int row, BatchClassPluginConfigDTO dto, boolean readOnly) {
		...
		if (!readOnly) {
			if (dto.getPluginConfig() != null
					&& !(dto.getPluginConfig().getFieldName().equalsIgnoreCase("imagemagick.open_input_image_parameters")
