...
public class ModuleView extends View<ModuleViewPresenter> {
	...
	public void createPluginList(Collection<BatchClassPluginDTO> plugins) {
		...
		pluginListView.listView.initTable(recordList.size(), recordList, true);
	}

	private List<Record> setPluginList(Collection<BatchClassPluginDTO> plugins) {
	...
	public class PluginListView {
		...
		HeaderColumn name = new HeaderColumn(1, LocaleDictionary.get().getConstantValue(BatchClassManagementConstants.NAME), 20);
		HeaderColumn description = new HeaderColumn(2, LocaleDictionary.get().getConstantValue(
