...
public class PageTypeEvent extends GwtEvent<PageTypeHandler> {
	...
	/**
	 * The page on which the user takes action.
	 */
	...
	public PageTypeEvent(final Action action, final PageTypeDTO pageTypeDTO) {
		super();
		this.action = action;
		this.pageTypeDTO = pageTypeDTO;
	}

	@Override
	protected void dispatch(final PageTypeHandler handler) {
		switch (action) {
			case EDIT:
				handler.onPageTypeEdit(this);
				break;
			case SELECT:
				handler.onPageTypeSelect(this);
			default:
				break;
		}
	}
