...
public enum BatchPriority {
	...
	public static BatchPriority getBatchPriority(int priority) {
		...
		BatchPriority batchPriority = null;
		boolean isFound = false;

		for (BatchPriority bP : BatchPriority.values()) {
			isFound = getBatchPriority(priority, bP);
			if (isFound) {
				batchPriority = bP;
				break;
			}
		...
		if (null == batchPriority) {
			String errMsg = "BatchPriority not found for the input priority.";
	...
	private static boolean getBatchPriority(int priority,
		...
		int maxRange = batchPriority.getUpperLimit();

		if (priority <= maxRange && priority >= minRange) {
			isFound = true;
		}
