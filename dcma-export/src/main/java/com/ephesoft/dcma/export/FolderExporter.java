...
public class FolderExporter implements ICommonConstants {
	...
	public void exportFiles(String batchInstanceID) throws JAXBException, DCMAApplicationException {
		...
		if (exportToFolderSwitch.equalsIgnoreCase(ON_EXPORT)) {
			...
			File exportToBatchFolder = new File(exportToFolder + File.separator + batchInstanceID);
			exportToBatchFolder.mkdir();
			if (!exportToBatchFolder.exists()) {
				throw new DCMABusinessException("Could not create folder=" + exportToFolder);
			}
			LOGGER.info("Created Export To Folder=" + exportToFolder);

			File xmlFile = batchSchemaService.getFile(batchInstanceID, batchInstanceID + "_" + BATCH_XML);

			if (!xmlFile.exists()) {
				throw new DCMABusinessException("Could not find batch.xml file in the folder " + fFolderToBeExported);
			}
			...
			if (documents != null && documents.getDocument() != null) {
				...
				File destinationXmlFile = new File(exportToBatchFolder.getPath() + File.separator + xmlFile.getName());
				try {
					FileUtils.copyFile(xmlFile, destinationXmlFile, false);
				} catch (IOException e) {
					LOGGER.error("Problem copying XML file : " + xmlFile, e);
					throw new DCMAApplicationException("Problem copying XML file : " + xmlFile, e);
				}
				...
				for (Document document : listOfDocuments) {
					...
					if (sMultiPagePdf != null && !sMultiPagePdf.isEmpty()) {
						...
						File fDestinationPdfFile = new File(exportToBatchFolder.getPath() + File.separator + sMultiPagePdf);
						// fSourcePdfFile.renameTo(fDestinationPdfFile);
						try {
					...
					if (sMultiPageTif != null && !sMultiPageTif.isEmpty()) {
						...
						File fDestinationTifFile = new File(exportToBatchFolder.getPath() + File.separator + sMultiPageTif);
						File fSourceTifFile = new File(fFolderToBeExported.getPath() + File.separator + sMultiPageTif);
						// fSourceTifFile.renameTo(fDestinationTifFile);
						try {
