...
/**
 * The <code>FileNameValidator</code> class represents
 * 
 * @author Ephesoft
 * @version 1.0
 * @see com.ephesoft.dcma.gwt.core.client.validator.Validator
 * 
 */
public class FileNameValidator implements Validator {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.ephesoft.dcma.gwt.core.client.validator.Validator#validate()
	 */
	@Override
	public boolean validate() {
		boolean isValid = false;
		if (value.getValue() != null) {
			isValid = isFileNameValid(value.getValue());
		}
		return isValid;
	}

	/**
	 * Method to check for invalid characters in file name {fileName}.
	 * 
	 * @param fileName {@link String} name of the file from which invalid characters are to be replaced.
	 * @return true if file name is valid, else false.
	 */
	private boolean isFileNameValid(final String fileName) {
		char[] invalidChars = CoreCommonConstants.INVALID_FILE_EXTENSIONS.toCharArray();
