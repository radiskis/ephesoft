...
public class BatchClassManagementPresenter extends AbstractBatchClassPresenter<BatchClassManagementView> implements PaginationListner {
						...
						controller.getRpcService().getBatchClasses(0, Table.VISIBLE_RECORD_COUNT, null,
								new AsyncCallback<List<BatchClassDTO>>() {
									...
									public void onSuccess(List<BatchClassDTO> batches) {
										...
										view.createBatchList(batches, count);
									...
									public void onFailure(Throwable arg0) {
										ConfirmationDialogUtil
												.showConfirmationDialogError(MessageConstants.BATCH_CLASS_LIST_ERROR_MSG);
									}
								});
					}
				});
	...
	public void showKVPPAddEditPluginView(BatchClassPluginDTO batchClassPluginDTO) {
		...
		controller.setSelectedPlugin(batchClassPluginDTO);
		showKVPPAddEditPluginView();
	}

	public void showKVPPAddEditPluginView() {
		batchClassBreadCrumbPresenter.createBreadCrumbForKVPPPlugin(controller.getPluginConfigDTO());
			...
			public void onSuccess(BatchClassDTO batchClass) {
				controller.getRpcService().cleanup(new AsyncCallback<Void>() {
					...
					public void onSuccess(Void arg0) {
						...
						final ConfirmationDialog saveConfirmationDialog = new ConfirmationDialog(Boolean.TRUE);
						saveConfirmationDialog.setDialogTitle(LocaleDictionary.get().getMessageValue(
								BatchClassManagementMessages.SUCCESS));
						saveConfirmationDialog.setMessage(LocaleDictionary.get().getMessageValue(
						...
						saveConfirmationDialog.center();
						saveConfirmationDialog.show();
						saveConfirmationDialog.okButton.setFocus(true);
	...
	public void onApplyClicked() {
		...
		controller.getRpcService().updateBatchClass(controller.getBatchClass(), new AsyncCallback<BatchClassDTO>() {
			...
			public void onSuccess(final BatchClassDTO batchClassDTO) {
				...
				final ConfirmationDialog applyConfirmationDialog = new ConfirmationDialog(Boolean.TRUE);
				applyConfirmationDialog.setMessage(LocaleDictionary.get().getMessageValue(
						BatchClassManagementMessages.APPLY_SUCCESSFUL));
				applyConfirmationDialog.setDialogTitle(LocaleDictionary.get().getMessageValue(BatchClassManagementMessages.SUCCESS));
				applyConfirmationDialog.addDialogListener(new DialogListener() {
				...
				applyConfirmationDialog.center();
				applyConfirmationDialog.show();
				applyConfirmationDialog.okButton.setFocus(true);
				ScreenMaskUtility.unmaskScreen();
			}
		});
	}
