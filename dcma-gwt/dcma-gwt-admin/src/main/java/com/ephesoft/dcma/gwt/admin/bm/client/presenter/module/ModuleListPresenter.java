...
import com.ephesoft.dcma.gwt.admin.bm.client.util.ModuleComparator;
...
public class ModuleListPresenter extends AbstractBatchClassPresenter<ModuleListView> implements PaginationListner {
	...
	public void onPagination(int startIndex, int maxResult, Order order) {
		...
		List<Record> moduleRecordList = getController().getMainPresenter().getView().getBatchClassView().setModuleList(
				newModuleDTOList);
		this.getView().getModuleListView().updateRecords(moduleRecordList, 0);
	}
