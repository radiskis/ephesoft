...
/**
 * This class is used to upload pdf file.
 * 
 * @author Ephesoft
 * @version 1.0
 ...
 *
 */
public class UploadPdfFile extends AbstractUploadFile {
	/**
	...
	public void uploadFile(String batchInstanceIdentifier, String rootFolder, String alfrescoAspectSwitch, Session session,
		...
		if (sMultiPagePdf != null && !sMultiPagePdf.isEmpty()) {
			...
			try {
				...
				try {
					doc = (org.apache.chemistry.opencmis.client.api.Document) session
							.getObjectByPath(CMISExportConstant.FOLDER_SEPARATOR + rootFolder + CMISExportConstant.FOLDER_SEPARATOR
									+ fSourcePdfFile.getName());
				} catch (CmisObjectNotFoundException e) {
					LOGGER.error(e.getMessage(), e);
				}

				String pdfUpdatedFileName = null;

				if (updatedCmisFileName != null && !updatedCmisFileName.isEmpty()) {
					pdfUpdatedFileName = updatedCmisFileName + FileType.PDF.getExtensionWithDot();
				}

				doc = uploadDocument(doc, session, batchInstanceFolder, fSourcePdfFile, document, batchClassIdentifier, true,
						batchInstanceIdentifier, pdfUpdatedFileName, documentDetails);
				...
				throw new DCMAApplicationException(CMISExportConstant.UNABLE_TO_UPLOAD_THE_DOCUMENT + fSourcePdfFile, e);
			}
		}
	}
